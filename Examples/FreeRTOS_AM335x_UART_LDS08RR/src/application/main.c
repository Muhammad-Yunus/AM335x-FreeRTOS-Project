...
int main(void)
{
    ...
    /* Display task renders each decoded scan as an ASCII map on UART0. */
    xTaskCreate(&vLDSDisplayTask, "LDSDisp", STACK_DEPTH_1536_WORDS, NULL, PRIORITY_UART1_DISPLAY, NULL);
    ConsoleUtilsPrintf("Display task created (prio=%d)\r\n", PRIORITY_UART1_DISPLAY);
