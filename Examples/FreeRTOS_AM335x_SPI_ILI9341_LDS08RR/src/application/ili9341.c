 ...
 *
 * The SPI0 transmit helpers (Spi0Init / Spi0TxByte / Spi0TxBuffer) and
...
#include "ili9341.h"
#include "fonts.h"
#include "FreeRTOS.h"   /* vTaskDelay() replaces TI delay() — no DMTimer7 */
#include "task.h"
...
static void ILI9341_Reset(void)
{
    ...
    vTaskDelay(pdMS_TO_TICKS(10));
    LcdRstHigh();
    vTaskDelay(pdMS_TO_TICKS(ILI9341_RESET_CANCEL_MS));
}
                      ...
                      uint16_t w, uint16_t h, uint16_t color)
{
    ...
    /* Build one row's worth of pixels in RGB565 MSB-LSB order, then
     * transmit row-by-row. This keeps RAM use to one row (~640 bytes
     * for 320 px) instead of an entire framebuffer. */
    hi = ILI9341_HI(color);
    lo = ILI9341_LO(color);
...
/* Scaled variant: each 5x7 glyph pixel becomes a scale x scale block. The
 * whole character cell is background-filled first, then the fg blocks. */
void ILI9341_DrawStringScaled(uint16_t x, uint16_t y,
                              const char *s, uint16_t fg, uint16_t bg,
                              uint8_t scale)
{
    uint16_t cur_x = x;
    ...
    while (*s)
    {
        ...
        ILI9341_FillRect(cur_x, y,
                         (uint16_t)(FONT5X7_WIDTH * scale),
                         (uint16_t)(FONT5X7_HEIGHT * scale), bg);
        for (col = 0; col < FONT5X7_WIDTH; col++)
        {
            bits = glyph[col];
            for (row = 0; row < FONT5X7_HEIGHT; row++)
            {
                if (bits & (uint8_t)(1U << row))
                {
                    ILI9341_FillRect(cur_x + (uint16_t)col * scale,
        ...
        cur_x = (uint16_t)(cur_x + (FONT5X7_WIDTH + 1) * scale);
        s++;
    }
}
